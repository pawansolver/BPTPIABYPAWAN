...
export default function EngineeringCollegesPage() {
  ...
  const fetchColleges = useCallback(async () => {
    console.log("[ENGINEERING] Fetching colleges...", { currentPage, searchTerm });
    setLoading(true);
    setError(null);
    try {
      const params = {
      ...
      console.log("[ENGINEERING] Request params:", params);

      const response = await getColleges(params);
      console.log("[ENGINEERING] API Response:", response);

      if (response.success) {
        console.log("[ENGINEERING] Success - Colleges count:", response.data.length);
        console.log("[ENGINEERING] Pagination:", response.pagination);
        setColleges(response.data);
        setPagination(response.pagination || null);
      } else {
        console.error("[ENGINEERING] API returned error:", response.message);
        setError(response.message || "Failed to fetch colleges");
      }
    } catch (err) {
      console.error("[ENGINEERING] Fetch exception:", err);
      setError(err instanceof Error ? err.message : "An error occurred");
    } finally {
      console.log("[ENGINEERING] Fetch complete, loading:", false);
      setLoading(false);
    }
        ...
        <div className="bg-white border-b border-gray-200 sticky top-[164px] z-40 shadow-sm">
          <div className="max-w-[1400px] mx-auto px-4 md:px-6 py-3.5 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <h1 className="text-[17px] font-bold text-slate-900 tracking-wide">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <a
                href="/pdf/engineering_colleges_2025.pdf"
                download
